% Define file names for hold time data
power_file = 'hold_time_power_data.txt';
area_file = 'hold_time_sweep_total_area.txt';
...
% Combine power and area data based on hold time
% Using 'HoldTime_ns_' as the key for merging (updated)
combined_data = outerjoin(power_data, area_data, 'MergeKeys', true, 'Keys', 'HoldTime_ns_');

% Extract relevant data
hold_time = combined_data.HoldTime_ns_;  % Hold Time in ns
...
disp(['Hold Time: ', num2str(hold_time(optimal_index)), ' ns']);
...
output_file = 'C:\SPB_Data\EEE468_Jan2024_byakc\Exp2_ALU_LAYERED_NEW\randomyet\CLA_Focus_For_project\cla_reports_parameters_efforthigh_slowvdd01lib_sweep\summarised_5parameters_stacked\hold\optimized_parameters_hold_time.txt';
fid = fopen(output_file, 'w');
fprintf(fid, 'Optimal Parameters:\n');
fprintf(fid, 'Hold Time: %.6f ns\n', hold_time(optimal_index));
...
figure;
scatter3(total_power, total_area, hold_time, 'filled');
xlabel('Total Power (nW)');
ylabel('Total Area (units)');
zlabel('Hold Time (ns)');
title('Total Power vs Area vs Hold Time');
grid on;

% Save the figure as a PNG file
plot_file = 'C:\SPB_Data\EEE468_Jan2024_byakc\Exp2_ALU_LAYERED_NEW\randomyet\CLA_Focus_For_project\cla_reports_parameters_efforthigh_slowvdd01lib_sweep\summarised_5parameters_stacked\hold\optimized_parameter_hold_time_sweep.png';
saveas(gcf, plot_file);
disp(['Graph saved as ', plot_file]);

% Save results if needed
save('optimal_parameters_hold_time.mat', 'hold_time', 'total_power', 'total_area', 'optimal_index');
