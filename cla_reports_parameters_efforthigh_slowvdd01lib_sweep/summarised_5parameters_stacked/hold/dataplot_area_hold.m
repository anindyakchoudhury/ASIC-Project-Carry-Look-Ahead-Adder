% MATLAB script to extract data based on hold times and plot it
...
total_areas = zeros(1, length(hold_times));

% Open the synthesis report file
report_file = 'area_hold_sweep_data.rpt';  % update with full path if needed
...
output_file = 'C:\SPB_Data\EEE468_Jan2024_byakc\Exp2_ALU_LAYERED_NEW\randomyet\CLA_Focus_For_project\cla_reports_parameters_efforthigh_slowvdd01lib_sweep\summarised_5parameters_stacked\hold\hold_time_sweep_total_area.txt';
fid_out = fopen(output_file, 'w');
fprintf(fid_out, 'Hold Time (ns)\tTotal Area\n');
for i = 1:length(hold_times)
    fprintf(fid_out, '%f\t%f\n', hold_times(i), total_areas(i));
end
fclose(fid_out);

% Plot the hold time vs total area
figure;
plot(hold_times, total_areas, '-o', 'LineWidth', 2, 'MarkerSize', 8);
title('Hold Time vs Total Area');
xlabel('Hold Time (ns)');
ylabel('Total Area');
grid on;

% Save the plot as a PNG file
saveas(gcf, 'C:\SPB_Data\EEE468_Jan2024_byakc\Exp2_ALU_LAYERED_NEW\randomyet\CLA_Focus_For_project\cla_reports_parameters_efforthigh_slowvdd01lib_sweep\summarised_5parameters_stacked\hold\hold_time_vs_total_area.png');
